...
const ListPage = () => {
  ...
  const [activeItem, setActiveItem] = useState<string>(listId || "unknown");
  ...
  // Gọi API khi component được mount hoặc khi listId thay đổi
  useEffect(() => {
    fetchListStats(); // Sử dụng API thống kê cho list cụ thể
    fetchTaskLists();
    fetchOverallStats(); // Cần thống kê tổng quan cho sidebar
    fetchWeeklyTasks(); // Lấy số task tuần
    setActiveItem(listId || "unknown");
  }, [dateQuery, listId, sortBy]); // Thêm sortBy vào dependency
  ...
  // Hàm gọi API và lấy thống kê tasks theo list cụ thể
  const fetchListStats = async () => {
    if (!listId) return;
    
    try {
      const res = await taskStatsAPI.getTaskStatsByList(listId, sortBy);
      setTaskBuffer(res.tasks);
      // Cập nhật số lượng task theo list cụ thể
      setActiveTaskCount(res.activeCount);
      setCompleteTaskCount(res.completeCount);
    } catch (error) {
      console.error("Lỗi xảy ra khi truy xuất list stats", error);
      toast.error("Lỗi xảy ra khi truy xuất thống kê danh sách");
    }
  };
  ...
  // Hàm gọi lại fetchListStats để làm mới danh sách task
  ...
  // Lọc tasks chỉ thuộc list hiện tại
  const listTasks = taskBuffer.filter((task: Task) => {
    const taskList = task.list || "unknown";
    return taskList === listId;
  });

  // Áp dụng filter trạng thái trên tasks đã lọc theo list
  const filteredTasks = listTasks.filter((task: Task) => {
